...
**Files:**
- Create: `src/offender-tracker.test.mjs`
- Reference: `src/offender-tracker.mjs` (110 lines)

The offender tracker uses D1 to maintain per-pubkey stats (total_scanned, flagged_count, banned_count, restricted_count) with a calculated risk_level.

- [ ] **Step 1: Write offender tracker tests**

```javascript
// src/offender-tracker.test.mjs
// ABOUTME: Tests for per-pubkey moderation statistics tracking
// ABOUTME: Covers UPSERT logic, risk level calculation, and stat aggregation
...
import { testPubkey } from './test/helpers.mjs';

describe('offender-tracker', () => {
  beforeEach(async () => {
    await initOffenderTable(env.BLOSSOM_DB);
    // Clear any existing data
    await env.BLOSSOM_DB.prepare('DELETE FROM uploader_stats').run();
  });

  describe('updateUploaderStats', () => {
    it('creates new row on first scan', async () => {
      const pubkey = testPubkey('1');
      await updateUploaderStats(env.BLOSSOM_DB, pubkey, 'SAFE');
      const stats = await getUploaderStats(env.BLOSSOM_DB, pubkey);
      expect(stats.total_scanned).toBe(1);
      expect(stats.flagged_count).toBe(0);
      expect(stats.banned_count).toBe(0);
    });

    it('increments flagged_count for REVIEW action', async () => {
      const pubkey = testPubkey('2');
      await updateUploaderStats(env.BLOSSOM_DB, pubkey, 'REVIEW');
      const stats = await getUploaderStats(env.BLOSSOM_DB, pubkey);
      expect(stats.flagged_count).toBe(1);
    });

    it('increments banned_count for PERMANENT_BAN', async () => {
      const pubkey = testPubkey('3');
      await updateUploaderStats(env.BLOSSOM_DB, pubkey, 'PERMANENT_BAN');
      const stats = await getUploaderStats(env.BLOSSOM_DB, pubkey);
      expect(stats.banned_count).toBe(1);
    });

    it('increments restricted_count for AGE_RESTRICTED', async () => {
      const pubkey = testPubkey('4');
      await updateUploaderStats(env.BLOSSOM_DB, pubkey, 'AGE_RESTRICTED');
      const stats = await getUploaderStats(env.BLOSSOM_DB, pubkey);
      expect(stats.restricted_count).toBe(1);
    });

    it('accumulates across multiple updates', async () => {
      const pubkey = testPubkey('5');
      await updateUploaderStats(env.BLOSSOM_DB, pubkey, 'SAFE');
      await updateUploaderStats(env.BLOSSOM_DB, pubkey, 'SAFE');
      await updateUploaderStats(env.BLOSSOM_DB, pubkey, 'AGE_RESTRICTED');
      const stats = await getUploaderStats(env.BLOSSOM_DB, pubkey);
      expect(stats.total_scanned).toBe(3);
      expect(stats.restricted_count).toBe(1);
    });
  });
  ...
  describe('getUploaderStats', () => {
    it('returns null for unknown pubkey', async () => {
      const stats = await getUploaderStats(env.BLOSSOM_DB, testPubkey('unknown'));
  ...
  describe('getUploaderRiskLevel', () => {
    it('returns "high" when banned_count > 0', async () => {
      const pubkey = testPubkey('6');
      await updateUploaderStats(env.BLOSSOM_DB, pubkey, 'PERMANENT_BAN');
      const level = await getUploaderRiskLevel(env.BLOSSOM_DB, pubkey);
      expect(level).toBe('high');
    });

    it('returns "high" when restricted_count >= 5', async () => {
      const pubkey = testPubkey('7');
      for (let i = 0; i < 5; i++) {
        await updateUploaderStats(env.BLOSSOM_DB, pubkey, 'AGE_RESTRICTED');
      }
      const level = await getUploaderRiskLevel(env.BLOSSOM_DB, pubkey);
    ...
    it('returns "normal" for unknown pubkey', async () => {
      const level = await getUploaderRiskLevel(env.BLOSSOM_DB, testPubkey('none'));
      expect(level).toBe('normal');
    });
  });
});
...
Run: `cd /Users/rabble/code/divine/divine-moderation-service-tests && npx vitest run src/offender-tracker.test.mjs`
Expected: All tests PASS

- [ ] **Step 3: Commit**

```bash
git add src/offender-tracker.test.mjs
git commit -m "test: add offender tracker stats and risk level tests"
```
